...
// Use MODEL_FILES everywhere instead of MODEL_FILES or GUN_FILES
const MODEL_FILES = ["pharoah.glb", "forest_set.glb", "raven.glb"];
const GLOBAL_MODEL_SCALE = 1; // fallback
const GLOBAL_VERTICAL_OFFSET = 0;
const AUTO_VERTICALIZE_DELAY = 1000;

// Per-model scale and axis adjustment (example values, tweak as needed)
const MODEL_MODEL_ADJUST = [
  // pharoah
  {
...
const PubgOutfit = () => {
  ...
  useEffect(() => {
    ...
    // Move camera a bit closer for better visibility and to fit all models
    ...
    MODEL_FILES.forEach((filename, idx) => {
      loader.load(
        `./model/${filename}`,
        (gltf) => {
          ...
          if (loadedCount === MODEL_FILES.length) {
            ...
            // Scale and center all models, then apply per-model adjustment
            loadedModels.forEach((model, i) => {
              // First, scale to fit viewport
              model.scale.setScalar(scale);

              // Then, apply per-model scale adjustment (axis-wise)
              const adj = MODEL_MODEL_ADJUST[i] || {};
              if (adj.scale) {
              ...
              // Then, apply per-model position adjustment
              if (adj.position) {
              ...
              // Apply per-model rotation adjustment
              if (adj.rotation) {
            ...
            if (isMobile() || window.innerWidth < 700) {
              loadedModels.forEach((model, i) => {
                ...
                // Apply per-model position adjustment again (in case layout overwrites)
                const adj = MODEL_MODEL_ADJUST[i] || {};
                if (adj.position) {
                  model.position.x += adj.position.x ?? 0;
                  model.position.y += adj.position.y ?? 0;
            ...
            targetRotations.current = loadedModels.map((_, i) => {
              // Use per-model initial rotation if provided
              const adj = MODEL_MODEL_ADJUST[i] || {};
              return {
                x: adj.rotation?.x ?? 0,
                y: adj.rotation?.y ?? 0,
    ...
    const handleResize = () => {
      ...
      if (modelsRef.current.length) {
        ...
        modelsRef.current.forEach((model, i) => {
          ...
          // Apply per-model position adjustment
          if (adj.position) {
            model.position.x += adj.position.x ?? 0;
            model.position.y += adj.position.y ?? 0;
            model.position.z += adj.position.z ?? 0;
        ...
        if (isMobile() || window.innerWidth < 700) {
          modelsRef.current.forEach((model, i) => {
            ...
            // Apply per-model position adjustment again
            const adj = MODEL_MODEL_ADJUST[i] || {};
            if (adj.position) {
              model.position.x += adj.position.x ?? 0;
              model.position.y += adj.position.y ?? 0;
    ...
    const handleTouchMove = (e) => {
      ...
      if (Math.abs(dx) > 40 && Math.abs(dx) > Math.abs(dy)) {
        setCurrentCharIdx((prev) =>
          dx < 0
            ? Math.min(MODEL_FILES.length - 1, prev + 1)
            : Math.max(0, prev - 1)
        );
        isSwiping = false;
      }
  ...
  return (
    <div style={responsiveContainerStyle}>
        ...
        <>
          ...
          <button
            style={{
              ...
              right: "1.2rem",
              visibility:
                currentCharIdx === MODEL_FILES.length - 1
                  ? "hidden"
                  : "visible",
            }}
      ...
      <canvas
        ...
        tabIndex={0}
        aria-label="3D PUBG MODELs"
      ></canvas>
    </div>
  );
};

export default PubgOutfit;
