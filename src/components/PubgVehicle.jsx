...
// Use Vehicle_FILES everywhere instead of MODEL_FILES or GUN_FILES
const Vehicle_FILES = ["jeep.glb", "buggy.glb", "dacia.glb"];
const GLOBAL_MODEL_SCALE = 1; // fallback
const GLOBAL_VERTICAL_OFFSET = 0;
const AUTO_VERTICALIZE_DELAY = 1000;

// Per-vehicle scale and axis adjustment (example values, tweak as needed)
const VEHICLE_MODEL_ADJUST = [
  // jeep
  {
...
const PubgVehicle = () => {
  ...
  useEffect(() => {
    ...
    // Move camera a bit closer for better visibility and to fit all vehicles
    ...
    Vehicle_FILES.forEach((filename, idx) => {
      loader.load(
        `./model/${filename}`,
        (gltf) => {
          ...
          loadedCount++;
          if (loadedCount === Vehicle_FILES.length) {
            ...
            // Scale and center all models, then apply per-vehicle adjustment
            loadedModels.forEach((model, i) => {
              // First, scale to fit viewport
              model.scale.setScalar(scale);

              // Then, apply per-vehicle scale adjustment (axis-wise)
              const adj = VEHICLE_MODEL_ADJUST[i] || {};
              if (adj.scale) {
              ...
              // Then, apply per-vehicle position adjustment
              if (adj.position) {
              ...
              // Apply per-vehicle rotation adjustment
              if (adj.rotation) {
            ...
            if (isMobile() || window.innerWidth < 700) {
              loadedModels.forEach((model, i) => {
                ...
                model.position.z = -box.center.z - (box.center.z - zCenter); // align Z center

                // Apply per-vehicle position adjustment again (in case layout overwrites)
                const adj = VEHICLE_MODEL_ADJUST[i] || {};
                if (adj.position) {
                  model.position.x += adj.position.x ?? 0;
                  model.position.y += adj.position.y ?? 0;
            ...
            targetRotations.current = loadedModels.map((_, i) => {
              // Use per-vehicle initial rotation if provided
              const adj = VEHICLE_MODEL_ADJUST[i] || {};
              return {
                x: adj.rotation?.x ?? 0,
                y: adj.rotation?.y ?? 0,
    ...
    const handleResize = () => {
      ...
      if (modelsRef.current.length) {
        ...
        modelsRef.current.forEach((model, i) => {
          ...
          // Apply per-vehicle position adjustment
          if (adj.position) {
            model.position.x += adj.position.x ?? 0;
            model.position.y += adj.position.y ?? 0;
            model.position.z += adj.position.z ?? 0;
        ...
        if (isMobile() || window.innerWidth < 700) {
          modelsRef.current.forEach((model, i) => {
            ...
            // Apply per-vehicle position adjustment again
            const adj = VEHICLE_MODEL_ADJUST[i] || {};
            if (adj.position) {
              model.position.x += adj.position.x ?? 0;
              model.position.y += adj.position.y ?? 0;
    ...
    const handleTouchMove = (e) => {
      ...
      if (Math.abs(dx) > 40 && Math.abs(dx) > Math.abs(dy)) {
        setCurrentCharIdx((prev) =>
          dx < 0
            ? Math.min(Vehicle_FILES.length - 1, prev + 1)
            : Math.max(0, prev - 1)
        );
        isSwiping = false;
      }
  ...
  return (
    <div style={responsiveContainerStyle}>
        ...
        <>
          ...
          <button
            style={{
              ...
              right: "1.2rem",
              visibility:
                currentCharIdx === Vehicle_FILES.length - 1
                  ? "hidden"
                  : "visible",
            }}
      ...
      <canvas
        ...
        tabIndex={0}
        aria-label="3D PUBG Vehicles"
      ></canvas>
    </div>
  );
};

export default PubgVehicle;
